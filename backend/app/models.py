...
class TestcaseReview(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True, autoincrement=True, comment='评审ID')
    testcase_id = db.Column(db.Integer, db.ForeignKey('testcases.id'), nullable=False, comment='测试用例ID')
    reviewer_id = db.Column(db.Integer, db.ForeignKey('users.id'), nullable=False, comment='评审人ID')
    
    # 评审结果
    status = db.Column(db.String(20), default='pending', comment='评审状态: pending-待评审, approved-通过, rejected-拒绝, need_revision-需要修改')
    overall_rating = db.Column(db.Integer, comment='整体评分: 1-5分')
    
    # 评审意见
    comments = db.Column(db.Text, comment='评审意见')
    improvement_suggestions = db.Column(db.Text, comment='改进建议')
    
    # 详细评分（可选）
    clarity_score = db.Column(db.Integer, comment='清晰度评分: 1-5分')
    completeness_score = db.Column(db.Integer, comment='完整性评分: 1-5分')
    feasibility_score = db.Column(db.Integer, comment='可执行性评分: 1-5分')
    coverage_score = db.Column(db.Integer, comment='覆盖度评分: 1-5分')
    
    # 时间戳
    created_at = db.Column(db.DateTime, default=datetime.utcnow, comment='创建时间')
    reviewed_at = db.Column(db.DateTime, comment='评审时间')
    
    # 关联关系
    testcase = db.relationship('TestCase', backref=db.backref('reviews', lazy='dynamic'))
    reviewer = db.relationship('User', backref=db.backref('testcase_reviews', lazy='dynamic'))
    
    def to_dict(self):
        return {
            'id': self.id,
            'testcase_id': self.testcase_id,
            'testcase_title': self.testcase.title if self.testcase else None,
...
class ReviewComment(db.Model):
    ...
    review_id = db.Column(db.Integer, db.ForeignKey('testcase_reviews.id'), nullable=False, comment='评审ID')
    ...
    # 关联关系
    review = db.relationship('TestcaseReview', backref=db.backref('review_comment_list', lazy='dynamic'))
...
class ReviewTemplate(db.Model):
    ...
    category = db.Column(db.String(50), default='general', comment='分类: general/web/api/performance/security')
    
    # 评审检查点（JSON格式）
    checklist = db.Column(db.Text, comment='评审检查点JSON')
    
    # 评分标准
    scoring_criteria = db.Column(db.Text, comment='评分标准JSON')
